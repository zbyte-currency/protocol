...
const Zbyte = require('../../dist/node.js');
...
if (!Zbyte.GenesisConfig.CONFIGS[config.network]) {
...
Zbyte.Log.instance.level = config.log.level;
for (const tag in config.log.tags) {
    Zbyte.Log.instance.setLoggable(tag, config.log.tags[tag]);
}

for (const key in config.constantOverrides) {
    Zbyte.ConstantHelper.instance.set(key, config.constantOverrides[key]);
}
...
(async () => {
    if (config.protocol === 'dumb') {
        Zbyte.Log.e(TAG, `******************************************************************************`);
        Zbyte.Log.e(TAG, `*                                                                            *`);
        Zbyte.Log.e(TAG, `*  You are running in 'dumb' configuration, so others can't connect to you.  *`);
        Zbyte.Log.e(TAG, `*  Consider switching to a proper WebSocket/WebSocketSecure configuration.   *`);
        Zbyte.Log.e(TAG, `*                                                                            *`);
        Zbyte.Log.e(TAG, `******************************************************************************`);
    }

    Zbyte.Log.i(TAG, `Zbyte NodeJS Client starting (network=${config.network}`
    ...
    Zbyte.GenesisConfig.init(Zbyte.GenesisConfig.CONFIGS[config.network]);

    for (const seedPeer of config.seedPeers) {
        let address;
        switch (seedPeer.protocol) {
            case 'ws':
                address = Zbyte.WsPeerAddress.seed(seedPeer.host, seedPeer.port, seedPeer.publicKey);
                break;
            case 'wss':
            default:
                address = Zbyte.WssPeerAddress.seed(seedPeer.host, seedPeer.port, seedPeer.publicKey);
                break;
        }
        Zbyte.GenesisConfig.SEED_PEERS.push(address);
    }

    let networkConfig;
    switch (config.protocol) {
        case 'wss':
            networkConfig = new Zbyte.WssNetworkConfig(config.host, config.port, config.tls.key, config.tls.cert, config.reverseProxy);
            break;
        case 'ws':
            networkConfig = new Zbyte.WsNetworkConfig(config.host, config.port, config.reverseProxy);
            break;
        case 'dumb':
            networkConfig = new Zbyte.DumbNetworkConfig();
            break;
    }

    switch (config.type) {
        case 'full':
            $.consensus = await Zbyte.Consensus.full(networkConfig);
            break;
        case 'light':
            $.consensus = await Zbyte.Consensus.light(networkConfig);
            break;
        case 'nano':
            $.consensus = await Zbyte.Consensus.nano(networkConfig);
            break;
    }
    ...
    Zbyte.Log.i(TAG, `Peer address: ${networkConfig.peerAddress.toString()} - public key: ${networkConfig.keyPair.publicKey.toHex()}`);

    // TODO: Wallet key.
    $.walletStore = await new Zbyte.WalletStore();
    if (!config.wallet.address && !config.wallet.seed) {
        ...
        const mainWallet = Zbyte.Wallet.loadPlain(config.wallet.seed);
        await $.walletStore.put(mainWallet);
        await $.walletStore.setDefault(mainWallet.address);
        $.wallet = mainWallet;
    } else {
        const address = Zbyte.Address.fromUserFriendlyAddress(config.wallet.address);
    ...
    Zbyte.Log.i(TAG, `Managing wallets [${addresses.map(address => address.toUserFriendlyAddress())}]`);

    const account = !isNano ? await $.accounts.get($.wallet.address) : null;
    Zbyte.Log.i(TAG, `Wallet initialized for address ${$.wallet.address.toUserFriendlyAddress()}.`
        + (!isNano ? ` Balance: ${Zbyte.Policy.satoshisToCoins(account.balance)} ZBT` : ''));

    Zbyte.Log.i(TAG, `Blockchain state: height=${$.blockchain.height}, headHash=${$.blockchain.headHash}`);

    const extraData = config.miner.extraData ? Zbyte.BufferUtils.fromAscii(config.miner.extraData) : new Uint8Array(0);
    if (config.poolMining.enabled || config.uiServer.enabled) { // ui requires SmartPoolMiner to be able to switch
        ...
        const deviceId = Zbyte.BasePoolMiner.generateDeviceId(networkConfig);
        const deviceData = config.poolMining.deviceData;
        const poolMode = isNano ? 'nano' : config.poolMining.mode;
        switch (poolMode) {
            case 'nano':
                $.miner = new Zbyte.NanoPoolMiner($.blockchain, $.network.time, $.wallet.address, deviceId, deviceData);
                break;
            case 'smart':
            default:
                $.miner = new Zbyte.SmartPoolMiner($.blockchain, $.accounts, $.mempool, $.network.time, $.wallet.address, deviceId, deviceData, extraData);
                break;
        }
        $.consensus.on('established', () => {
            if (!config.poolMining.enabled || !$.miner.isDisconnected()) return;
            if (!config.poolMining.host || config.poolMining.port === -1) {
                Zbyte.Log.i(TAG, 'Not connecting to pool as mining pool host or port were not specified.');
                return;
            }
            Zbyte.Log.i(TAG, `Connecting to pool ${config.poolMining.host} using device id ${deviceId} as a ${poolMode} client.`);
            $.miner.connect(config.poolMining.host, config.poolMining.port);
        });
    } else {
        $.miner = new Zbyte.Miner($.blockchain, $.accounts, $.mempool, $.network.time, $.wallet.address, extraData);
    }

    $.blockchain.on('head-changed', (head) => {
        if ($.consensus.established || head.height % 100 === 0) {
            Zbyte.Log.i(TAG, `Now at block: ${head.height}`);
        }
    });

    $.network.on('peer-joined', (peer) => {
        Zbyte.Log.i(TAG, `Connected to ${peer.peerAddress.toString()}`);
    });
    $.network.on('peer-left', (peer) => {
        Zbyte.Log.i(TAG, `Disconnected from ${peer.peerAddress.toString()}`);
    });

    const isSeed = (peerAddress) => Zbyte.GenesisConfig.SEED_PEERS.some(seed => seed.equals(peerAddress));
    $.network.on('peer-joined', (peer) => {
        if (Math.abs(peer.timeOffset) > Zbyte.Network.TIME_OFFSET_MAX && isSeed(peer.peerAddress)) {
            Zbyte.Log.e(TAG, 'Your local system time seems to be wrong! You might not be able to synchronize with the network.');
        }
    });
    ...
    $.consensus.on('established', () => {
        Zbyte.Log.i(TAG, `Blockchain ${config.type}-consensus established in ${(Date.now() - START) / 1000}s.`);
        Zbyte.Log.i(TAG, `Current state: height=${$.blockchain.height}, totalWork=${$.blockchain.totalWork}, headHash=${$.blockchain.headHash}`);
    });

    $.miner.on('block-mined', (block) => {
        Zbyte.Log.i(TAG, `Block mined: #${block.header.height}, hash=${block.header.hash()}`);
    });
    ...
    if (config.statistics > 0) {
        ...
        $.miner.on('hashrate-changed', async (hashrate) => {
            ...
            if (hashrates.length >= outputInterval) {
                ...
                Zbyte.Log.i(TAG, `Hashrate: ${(sum / hashrates.length).toFixed(2).padStart(7)} H/s`
                    + (!isNano ? ` - Balance: ${Zbyte.Policy.satoshisToCoins(account.balance)} ZBT` : '')
                    + (config.poolMining.enabled ? ` - Pool balance: ${Zbyte.Policy.satoshisToCoins($.miner.balance)} ZBT (confirmed ${Zbyte.Policy.satoshisToCoins($.miner.confirmedBalance)} ZBT)` : '')
    ...
    if (config.uiServer.enabled) {
        ...
        openBrowserTab(`http://localhost:${config.uiServer.port}#port=${config.rpcServer.port}`, () => {
            Zbyte.Log.w(TAG, 'Failed to automatically open the UI in your web browser.');
            Zbyte.Log.w(TAG, `Go to http://localhost:${config.uiServer.port}#port=${config.rpcServer.port} to access it.`);
        });
    }
})().catch(e => {
    console.error(e);
